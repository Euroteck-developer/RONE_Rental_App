    ...
    if (result.rows.length === 0) {
      await query(
        `INSERT INTO security_events (event_type, severity, ip_address, details)
         VALUES ($1, $2, $3, $4)`,
        ['FAILED_LOGIN', 'MEDIUM', ipAddress, JSON.stringify({ email })]
      ).catch(() => {}); // Don't fail login flow if security log fails
      return res.status(401).json({ success: false, error: 'Invalid credentials' });
    }
...
const forgotPassword = async (req, res) => {
  try {
    ...
    try {
      ...
      console.error('[forgotPassword] DB error (users lookup):', dbErr.message);
    ...
    // ── 2. Rate limit: max 3 OTPs per 10 minutes ────────────
    let rateLimitResult;
    try {
      rateLimitResult = await query(
        `SELECT COUNT(*) FROM password_reset_otps
         WHERE email = $1 AND created_at > NOW() - INTERVAL '10 minutes'`,
        [normalizedEmail]
      );
    } catch (dbErr) {
      console.error('[forgotPassword] DB error (rate limit check):', dbErr.message);
      return res.status(500).json({ success: false, error: 'Database error. Please try again.' });
    }
    ...
    // ── 4. Invalidate old OTPs and insert new one ────────────
    try {
      await query(
      ...
      console.error('[forgotPassword] DB error (insert OTP):', dbErr.message);
    ...
    try {
      ...
      await query(
        `UPDATE password_reset_otps SET used = true WHERE email = $1 AND otp_hash = $2`,
...
const verifyOTP = async (req, res) => {
  try {
    ...
    if (!result.rows.length) {
      // Track failed attempt on the latest unused record (if any)
      await query(
        `UPDATE password_reset_otps
         SET failed_attempts = COALESCE(failed_attempts, 0) + 1
         WHERE email = $1 AND used = false`,
    ...
    await query(
      `INSERT INTO password_reset_tokens (user_id, token_hash, email, expires_at)
       VALUES ($1, $2, $3, $4)`,
      [record.user_id, resetTokenHash, normalizedEmail, resetExpiresAt]
    );
...
const resetPassword = async (req, res) => {
  try {
    ...
    try {
      ...
      console.warn('[resetPassword] Confirmation email failed:', emailErr.message);
    }
