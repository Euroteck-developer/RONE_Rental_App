...
// ─── KEY: unit_no is resolved from customer_units via a lateral join.
//     Each payment row is linked to customer_units through customer_unit_id
//     (if available) or by matching customer_id + falling back to the most
//     recent active unit for that customer.
// ─────────────────────────────────────────────────────────────────────────────
const UNIT_JOIN = `
  LEFT JOIN LATERAL (
    SELECT cu.unit_no, cu.floor_no
    FROM customer_units cu
    WHERE cu.deleted_at IS NULL
      AND (
        cu.id = p.customer_unit_id
        OR (p.customer_unit_id IS NULL AND cu.customer_id = c.id)
      )
    ORDER BY (cu.id = p.customer_unit_id) DESC, cu.created_at DESC
    LIMIT 1
  ) cu ON TRUE
`;
...
const getQuarterlyTDS = async (req, res) => {
  try {
    ...
    // Customer breakdown — includes unit_no from customer_units
    const customerResult = await query(
      `SELECT c.id AS customer_id, c.customer_name, c.customer_id AS customer_code,
        c.pan_number, c.agreement_type, c.gst_no, c.cgst, c.sgst, c.nri_status,
        cu_agg.unit_no,
       ...
       FROM customers c
       JOIN payments p ON c.id = p.customer_id AND p.deleted_at IS NULL
       -- Resolve unit_no: prefer unit linked to first matching payment, else latest active unit
       LEFT JOIN LATERAL (
         SELECT cu.unit_no
         FROM customer_units cu
         WHERE cu.deleted_at IS NULL
           AND (
             cu.id = (
               SELECT p2.customer_unit_id FROM payments p2
               WHERE p2.customer_id = c.id
                 AND p2.payment_month = ANY($1)
                 AND p2.deleted_at IS NULL
               LIMIT 1
             )
             OR (
               NOT EXISTS (
                 ...
                 WHERE p3.customer_id = c.id
                   AND p3.customer_unit_id IS NOT NULL
                   AND p3.payment_month = ANY($1)
...
const generateCertificate = async (req, res) => {
  try {
    ...
    const customerResult = await query(
      `SELECT c.*, fr.tds_applicable, fr.rental_value_per_sft, fr.payment_closure_date AS fr_closure_date,
              cu.unit_no, cu.floor_no AS unit_floor_no
       FROM customers c
       LEFT JOIN ${LATEST_FR} fr ON c.id = fr.customer_id
       LEFT JOIN LATERAL (
         SELECT cu2.unit_no, cu2.floor_no
         FROM customer_units cu2
         WHERE cu2.customer_id = c.id AND cu2.deleted_at IS NULL
...
const downloadCertificate = async (req, res) => {
  try {
    ...
    doc.text(`Customer ID: ${certificate.customer_code}`);
    doc.text(`PAN Number:  ${certificate.pan_number}`);
    doc.text(`Unit No:     ${certificate.unit_no || '—'}`);
    doc.text(`Email:       ${certificate.email || '—'}`);
    if (isNRIFlag) doc.text('NRI Status:  Non-Resident Indian (NRI)');
    if (hasGST) {
      doc.moveDown(0.5);
