...
// ──────────────────────────────────────────────────────────────────────────────
// After the multi-unit migration, floor_no and agreement_type live in
// customer_units. The customers table still has them for existing rows but
// new customers added after the migration will only have them in customer_units.
// All queries below join BOTH tables and COALESCE so both old and new records work.
// ──────────────────────────────────────────────────────────────────────────────

/* Shared CTE that picks the most-recent active unit for each customer.
   Used in every query that needs floor_no / agreement_type / actual_occupancy_date. */
const UNIT_CTE = `
  WITH latest_unit AS (
    SELECT DISTINCT ON (customer_id)
      customer_id,
      ...
      base_rent_9_year,
      property_name  AS unit_property_name,
  ...
  try {
    ...
    let queryText = `
      ...
      SELECT
        ...
        c.customer_name,
        c.customer_id  AS customer_code,
        COALESCE(lu.unit_property_name, c.property_name)   AS property_name,
        COALESCE(lu.floor_no,           c.floor_no)        AS floor_no,
        COALESCE(lu.agreement_type,     c.agreement_type)  AS agreement_type,
        COALESCE(lu.actual_occupancy_date, c.actual_occupancy_date) AS actual_occupancy_date,
        c.email, c.phone
      FROM escalations e
      JOIN customers   c  ON e.customer_id = c.id
      LEFT JOIN latest_unit lu ON lu.customer_id = c.id
      WHERE e.deleted_at IS NULL
        AND COALESCE(lu.agreement_type, c.agreement_type) = '9-Year'
        AND COALESCE(lu.floor_no,       c.floor_no)       = '7'
    `;

    if (status) { queryText += ` AND e.status = $${p}`;           params.push(status); p++; }
    if (type)   { queryText += ` AND e.escalation_type = $${p}`;  params.push(type);   p++; }
    ...
    console.error('getAllEscalations error:', error);
...
const getUpcomingEscalations = async (req, res) => {
  try {
    ...
    const result = await query(`
      ...
      SELECT
        ...
        EXTRACT(YEAR FROM AGE(CURRENT_DATE,
          COALESCE(lu.actual_occupancy_date, c.actual_occupancy_date)))::INTEGER AS years_elapsed,

        CASE
          WHEN EXTRACT(YEAR FROM AGE(CURRENT_DATE, COALESCE(lu.actual_occupancy_date, c.actual_occupancy_date))) >= 6 THEN 'SECOND'
          WHEN EXTRACT(YEAR FROM AGE(CURRENT_DATE, COALESCE(lu.actual_occupancy_date, c.actual_occupancy_date))) >= 3 THEN 'FIRST'
          ELSE NULL
        END AS escalation_type,

        CASE
          WHEN EXTRACT(YEAR FROM AGE(CURRENT_DATE, COALESCE(lu.actual_occupancy_date, c.actual_occupancy_date))) >= 6
            THEN ROUND(fr.rent * 1.3225, 2)
          WHEN EXTRACT(YEAR FROM AGE(CURRENT_DATE, COALESCE(lu.actual_occupancy_date, c.actual_occupancy_date))) >= 3
            THEN ROUND(fr.rent * 1.15, 2)
          ELSE fr.rent
        END AS new_rent,
        ...
        CASE
          WHEN EXTRACT(YEAR FROM AGE(CURRENT_DATE, COALESCE(lu.actual_occupancy_date, c.actual_occupancy_date))) >= 6
            THEN COALESCE(lu.actual_occupancy_date, c.actual_occupancy_date) + INTERVAL '6 years'
          WHEN EXTRACT(YEAR FROM AGE(CURRENT_DATE, COALESCE(lu.actual_occupancy_date, c.actual_occupancy_date))) >= 3
            THEN COALESCE(lu.actual_occupancy_date, c.actual_occupancy_date) + INTERVAL '3 years'
          ELSE NULL
        END AS escalation_date
      ...
      WHERE c.deleted_at IS NULL
        AND COALESCE(lu.unit_status,    c.status)         = 'Active'
        AND COALESCE(lu.agreement_type, c.agreement_type) = '9-Year'
        AND COALESCE(lu.floor_no,       c.floor_no)       = '7'
        AND COALESCE(lu.actual_occupancy_date, c.actual_occupancy_date) IS NOT NULL
        AND fr.rent IS NOT NULL
        AND fr.rent > 0
        AND EXTRACT(YEAR FROM AGE(CURRENT_DATE,
            COALESCE(lu.actual_occupancy_date, c.actual_occupancy_date))) >= 3
        AND (
          COALESCE(lu.actual_occupancy_date, c.actual_occupancy_date) + INTERVAL '3 years'
            <= CURRENT_DATE + ($1 || ' months')::INTERVAL
          OR
          COALESCE(lu.actual_occupancy_date, c.actual_occupancy_date) + INTERVAL '6 years'
            <= CURRENT_DATE + ($1 || ' months')::INTERVAL
        )
      ORDER BY escalation_date ASC
    `, [months]);
    ...
    console.error('getUpcomingEscalations error:', error);
    return res.status(500).json({ success: false, error: 'Failed to fetch upcoming escalations' });
  }
};
...
const getEscalationByCustomer = async (req, res) => {
  try {
    ...
    console.error('getEscalationByCustomer error:', error);
    return res.status(500).json({ success: false, error: 'Failed to fetch customer escalations' });
  }
};
...
const createEscalation = async (req, res) => {
  try {
    ...
    // Check via customer_units first, fall back to customers
    ...
    if (dupCheck.rows.length > 0)
      return res.status(400).json({
        success: false,
        error: `A ${escalationType} escalation already exists for this customer`,
      });
    ...
    return res.status(500).json({ success: false, error: 'Failed to create escalation' });
  }
};
...
const applyEscalation = async (req, res) => {
  try {
    ...
    await transaction(async (client) => {
      ...
      if (!rows.length)          throw new Error('Escalation not found');
      const esc = rows[0];
      if (esc.agreement_type !== '9-Year') throw new Error('Escalations only apply to 9-Year rental customers');
      if (esc.floor_no !== '7')            throw new Error('Escalations only apply to Floor 7 customers');
      if (!esc.actual_occupancy_date)      throw new Error('Customer must have an actual occupancy date');
      ...
      // Update customers.base_rent_9_year (legacy)
      await client.query(
        `UPDATE customers SET base_rent_9_year=$1, updated_at=NOW(), updated_by=$2 WHERE id=$3`,
        [esc.new_rent, userId, esc.customer_id]
      );
      // Also update customer_units.base_rent_9_year for the latest unit
      if (esc.unit_id) {
        await client.query(
          `UPDATE customer_units SET base_rent_9_year=$1, updated_at=NOW(), updated_by=$2 WHERE id=$3`,
          [esc.new_rent, userId, esc.unit_id]
        );
      }
    ...
    return res.json({ success: true, message: 'Escalation applied successfully' });
  } catch (error) {
    console.error('applyEscalation error:', error);
    return res.status(500).json({ success: false, error: error.message || 'Failed to apply escalation' });
  }
};
...
const getEscalationTimeline = async (req, res) => {
  try {
    ...
    console.error('getEscalationTimeline error:', error);
    return res.status(500).json({ success: false, error: 'Failed to fetch escalation timeline' });
  }
};

const getEscalationStats = async (_req, res) => {
  try {
    const result = await query(`
      ${UNIT_CTE}
      SELECT
        ...
        COUNT(DISTINCT c.id) FILTER (
          WHERE COALESCE(lu.agreement_type, c.agreement_type) = '9-Year'
        )                                                    AS total_9year_customers,
        COUNT(DISTINCT c.id) FILTER (
          WHERE COALESCE(lu.agreement_type, c.agreement_type) = 'Construction'
        )                                                    AS total_construction_customers,
        COUNT(DISTINCT c.id) FILTER (
          WHERE COALESCE(lu.agreement_type, c.agreement_type) = '9-Year'
            AND COALESCE(lu.floor_no, c.floor_no) = '7'
            AND COALESCE(lu.actual_occupancy_date, c.actual_occupancy_date) IS NOT NULL
      ...
      RIGHT JOIN customers c ON e.customer_id = c.id AND e.deleted_at IS NULL
      LEFT JOIN latest_unit lu ON lu.customer_id = c.id
      WHERE c.deleted_at IS NULL AND COALESCE(lu.unit_status, c.status) = 'Active'
    `);
    return res.json({ success: true, data: result.rows[0] });
  } catch (error) {
    console.error('getEscalationStats error:', error);
    return res.status(500).json({ success: false, error: 'Failed to fetch escalation statistics' });
  }
};
...
const generateEscalations = async (req, res) => {
  try {
    ...
    await transaction(async (client) => {
      ...
      if (eligible.length === 0) {
        throw new Error(
          ...
          'Ensure Floor 7 / 9-Year customers have a financial record with rent > 0 and actual occupancy date set.'
        );
      }

      // Build FIRST and SECOND for customers with >= 3 or >= 6 years
      const escalationsToCreate = [];
      ...
      for (const item of escalationsToCreate) {
        ...
        const { rows: existing } = await client.query(`
          SELECT id, status FROM escalations
          WHERE customer_id = $1 AND escalation_type = $2 AND deleted_at IS NULL
        `, [customer.id, fields.escalation_type]);
        ...
        `, [
          newEsc.id, customer.id,
          customer.current_rent, fields.new_rent, fields.increase_amount,
          userId,
          `Auto-generated | ${fields.escalation_type} | ${fields.escalation_period} | ` +
          `Occupancy: ${customer.actual_occupancy_date} | Years elapsed: ${customer.years_elapsed}`,
        ]);
      ...
      `, [
        userId,
        JSON.stringify({
          generated: generatedEscalations.length,
    ...
    console.error('generateEscalations error:', error);
    return res.status(500).json({ success: false, error: error.message || 'Failed to generate escalations' });
  }
};
