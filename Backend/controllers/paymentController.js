...
// ─── Partial tranche rent ─────────────────────────────────────────────────────
// Formula: (amountReceived / totalSale) × (sqft × rentPerSFT) × (daysCharged / totalDaysInMonth)
// daysCharged = remaining days of closure month when generating for closure month,
//               full month days for all subsequent months.
// This EXACTLY matches what financial.controller stores in partial_payments[i].rent.
const calcPartialBaseRent = (amountReceived, entryClosure, totalSale, sqft, rentPerSFT, rentMonth) => {
  const q = toFloat(sqft), r = toFloat(rentPerSFT);
  if (!q || !r) return 0;
  const a = toFloat(amountReceived), s = toFloat(totalSale);
...
// ─── Core gross computation (FULL and 9-Year only) ────────────────────────────
// NOTE: This function is ONLY for full-payment mode customers.
// For partial-payment customers, use calcPartialBaseRent() per tranche instead.
// Calling this for a partial customer would return sqft × rentPerSFT (the full
// monthly amount) which is WRONG — it ignores the amountReceived / totalSale ratio.
const computeGrossForCustomer = (customer, closureDate, rentMonth) => {
  const rentPerSft  = toFloat(customer.rental_value_per_sft);
  const sqft        = toFloat(customer.sqft);
  const monthlyRent = sqft && rentPerSft ? round2(sqft * rentPerSft) : 0;
...
const FR_JOIN_FOR_PAYMENT = `
  LEFT JOIN LATERAL (
    ...
    FROM financial_records
    WHERE deleted_at IS NULL
      AND customer_unit_id = CASE
            WHEN p.customer_unit_id IS NOT NULL THEN p.customer_unit_id
            ELSE cu_direct.id
          END
    ORDER BY created_at DESC
    LIMIT 1
...
const insertPayment = async (client, params) => {
  ...
  const { rows: [p] } = await client.query(
    `INSERT INTO payments (
     ...
     SELECT
       $1::uuid, $2::date, $3::varchar, $4::numeric, $5::numeric,
       $6::numeric, $7::varchar, $8::numeric, $9::numeric, $10::numeric,
       $11::date, 'Pending'::varchar, $12::uuid,
       $13::int, $14::int, $15::numeric,
       $16::jsonb, $17::uuid
     WHERE NOT EXISTS (
       SELECT 1 FROM payments
       WHERE  customer_unit_id = $17::uuid
     ...
     RETURNING *`,
    [
      customerId, paymentDate, rentMonth,
      grossAmount, tdsAmount, netPayout,
      period, baseRent, escalationRate, yearsElapsed,
...
// ─── SQL for fetching ALL units of a customer ─────────────────────────────────
const ALL_UNITS_FOR_CUSTOMER_SQL = `
  SELECT
    c.id AS id, c.customer_id, c.customer_name, c.email, c.phone,
    c.gst_no, c.cgst, c.sgst,
  ...
  LEFT JOIN LATERAL (
    SELECT rent, tds_applicable, rental_value_per_sft,
           ...
           payment_mode, partial_payments
    FROM financial_records
    WHERE deleted_at IS NULL AND customer_unit_id = cu.id
    ORDER BY created_at DESC LIMIT 1
  ) fr ON TRUE
  WHERE c.id = $1
    AND c.deleted_at IS NULL AND cu.deleted_at IS NULL
    AND c.status = 'Active' AND cu.status = 'Active'
  ORDER BY cu.id ASC
`;

// ═══════════════════════════════════════════════════════════════════════════════
//  _processPartialCustomer
//  Shared logic for both generateMonthlyPayments and createPaymentSchedule.
//
//  CRITICAL FIX — WHY THIS EXISTS:
//  Previously, when payMode === 'partial' but parseFinancialPartials() returned
//  null (e.g. partial_payments column is NULL or malformed JSON in the DB), the
//  code fell through silently to the FULL payment block and generated:
//
//    grossAmount = sqft × rental_value_per_sft   (e.g. 40 × 900 = ₹36,000)
//
//  instead of the correct partial-tranche amount (e.g. ₹1,200).
//  This is the exact bug causing Ambica Dushetti's May 2026 ₹36,000 row.
//
//  The fix: extract all partial handling into this function which returns an
//  explicit { handled: true/false, error } result. The callers ONLY fall through
//  to FULL mode when payMode !== 'partial'. If payMode === 'partial' and
//  parsing fails, the customer is SKIPPED with an error — never silently
//  promoted to full-payment calculation.
// ═══════════════════════════════════════════════════════════════════════════════
const _processPartialCustomer = async (client, {
  cust, rentMonth, scheduledDate, month, pmStr,
  tdsExemptC, splitsJson, cuId, period, userId, payments, skip,
}) => {
  const parsed = parseFinancialPartials(cust.partial_payments);

  if (!parsed) {
    // partial_payments is null/empty/malformed — do NOT fall through to full mode.
    // This customer's financial record is incomplete; skip with a clear message.
    skip(
      `Payment mode is 'partial' but no valid partial_payments data found. ` +
      `Please update the financial record for unit ${cust.unit_no} and re-generate.`
    );
    return { handled: true };
  }

  // ── Financial tranches (amountReceived-based) ────────────────────────────
  if (parsed.type === 'financial') {
    const allEntries = parsed.entries;
    const totalSaleC = toFloat(cust.total_sale_consideration);
    const sqftC      = toFloat(cust.sqft);
    ...
    if (!entries.length) {
      skip('No active partial tranches for this rent month');
      return { handled: true };
    }

    // Reference closure date for escalation calculation
    let cdfg = cust.payment_closure_date ? new Date(cust.payment_closure_date) : null;
    if (!cdfg) {
      for (const e of allEntries) {
        const src = getEntryClosureDate(e);
    ...
    // escalationRate/yearsElapsed from computeGrossForCustomer are safe to use
    // here because they do not depend on the full-month amount — they only
    // compute the escalation tier (0 / 15% / 32.25%) based on years elapsed.
    const { escalationRate, yearsElapsed } = computeGrossForCustomer(cust, cdfg, rentMonth);
    ...
    const entryData = entries.map((e) => {
      ...
      // calcPartialBaseRent uses the SAME formula as financial.controller:
      //   (amountReceived / totalSale) × (sqft × rentPerSFT) × (daysCharged / totalDays)
      // This guarantees payment.controller amounts match financial.controller amounts.
      const baseRent   = calcPartialBaseRent(amtRcvd, closureS, totalSaleC, sqftC, rpsftC, rentMonth);
      const entryGross = round2(baseRent + (escalationRate > 0 ? round2(baseRent * escalationRate / 100) : 0));
      return { closureS, dateStr: e.date ?? (scheduledDate || `${month}-01`), baseRent, entryGross };
    });

    // TDS applied on combined gross, charged to first row only
    const combinedGross = round2(entryData.reduce((s, d) => s + d.entryGross, 0));
    const combinedTds   = tdsExemptC ? 0 : calculateTDS(combinedGross);
  ...
  if (parsed.type === 'installment') {
    const refDate = cust.payment_closure_date
      ? new Date(cust.payment_closure_date)
      : new Date(scheduledDate || `${month || pmStr}-01`);
    const { grossAmount, escalationRate, yearsElapsed } =
      computeGrossForCustomer(cust, refDate, rentMonth);

    let defs;
    try { defs = calcPartialInstallments(cust.partial_payments); }
    catch (e) { skip(e.message); return { handled: true }; }

    if (!defs) {
      skip('Installment definitions could not be parsed');
      return { handled: true };
    }

    const bd = buildInstallmentBreakdown(defs, grossAmount, tdsExemptC);
    for (const inst of bd) {
      const instDate = buildScheduledDate(month || pmStr, inst.due_day);
      const p = await insertPayment(client, {
        customerId: cust.id,
        paymentDate: instDate,
        rentMonth,
        grossAmount: inst.gross_amount,
        tdsAmount:   inst.tds_amount,
        netPayout:   inst.net_payout,
        period,
        baseRent:    grossAmount,
        escalationRate,
        yearsElapsed,
        scheduledDate: instDate,
        userId,
        installmentNo:     inst.installment_no,
        totalInstallments: bd.length,
        installmentPct:    inst.percentage,
        payoutSplitsJson:  splitsJson,
        customerUnitId:    cuId,
      });
  ...
  // Unknown type — skip safely, never fall through to full
  skip(`Unrecognised partial_payments structure for unit ${cust.unit_no}`);
  return { handled: true };
};
...
const calculatePayment = async (req, res) => {
  try {
    ...
    if (customerUnitId) {
      const { rows } = await query(
        `SELECT
           ...
           c.gst_no, c.cgst, c.sgst,
           cu.sqft, cu.floor_no, cu.unit_no,
           cu.agreement_type, cu.actual_occupancy_date, cu.status,
           COALESCE(cu.payout_splits, c.payout_splits)         AS payout_splits,
           COALESCE(fr.tds_applicable, cu.tds_applicable, 'N') AS tds_applicable,
           fr.rent                                             AS financial_rent,
           fr.rental_value_per_sft, fr.total_sale_consideration,
           fr.payment_closure_date, fr.payment_mode, fr.partial_payments
         FROM customer_units cu
         JOIN customers c ON cu.customer_id = c.id
         LEFT JOIN LATERAL (
           SELECT rent, tds_applicable, rental_value_per_sft,
      ...
      const cust = rows[0];
      if (cust.status !== 'Active')
        return res.status(400).json({ success: false, error: `Unit is ${cust.status} — cannot calculate payment` });

      const unitCalc = await _calcSingleUnit(cust, paymentDate, rentMonth);
      if (unitCalc.error)
        return res.status(400).json({ success: false, error: unitCalc.error, code: unitCalc.code, ...unitCalc.meta });
    ...
    const { rows: allUnits } = await query(ALL_UNITS_FOR_CUSTOMER_SQL, [customerId]);
    if (!allUnits.length)
      return res.status(404).json({ success: false, error: 'Customer not found, inactive, or has no active units' });

    const results = [], errors = [];
    for (const cust of allUnits) {
      const unitCalc = await _calcSingleUnit(cust, paymentDate, rentMonth);
      if (unitCalc.error) {
        errors.push({ customerUnitId: cust.customer_unit_id, unitNo: cust.unit_no, error: unitCalc.error, code: unitCalc.code });
      } else {
        results.push(unitCalc.data);
      }
    }
    ...
    if (results.length === 0 && errors.length > 0)
      return res.status(400).json({ success: false, error: errors[0].error, code: errors[0].code, unitErrors: errors });

    return res.json({ success: true, multiUnit: true, data: results, unitErrors: errors.length > 0 ? errors : undefined });
...
const _calcSingleUnit = async (cust, paymentDate, rentMonth) => {
  ...
  const { rows: existingPayments } = await query(
    `SELECT id, status FROM payments
     WHERE customer_unit_id = $1 AND payment_month = $2
  ...
  if (startMonthKey && rentMonth < startMonthKey)
    return {
      error: `Payment for ${cust.customer_name} (Unit ${cust.unit_no}) has not started yet. Rent begins from ${toMonthLabel(startMonthKey)}.`,
      code:  'PAYMENT_NOT_STARTED',
      meta:  { startMonth: startMonthKey, startMonthLabel: toMonthLabel(startMonthKey), customerName: cust.customer_name, rentMonth },
    };

  const { rows: [pr] } = await query(
    `SELECT COALESCE(SUM(gross_amount), 0) AS total_paid
     FROM payments
     WHERE customer_unit_id = $1 AND deleted_at IS NULL AND status <> 'Cancelled'`,
    [cust.customer_unit_id]
  );
  const totalAlreadyPaid = toFloat(pr.total_paid);
  const remainingBalance = Math.max(0, totalSale - totalAlreadyPaid);
  ...
  const buildGstDetails = (netAmount) => {
    if (!hasGst)
      return { cgstAmount: 0, sgstAmount: 0, totalGstAmount: 0, totalInvoice: round2(netAmount), hasGst, gstNo, cgstRate, sgstRate };
    const { cgst: cgstAmount, sgst: sgstAmount, total: totalGstAmount } = calculateGSTSplit(netAmount, cgstRate, sgstRate);
    return { cgstAmount, sgstAmount, totalGstAmount, totalInvoice: round2(netAmount + totalGstAmount), hasGst, gstNo, cgstRate, sgstRate };
  };

  // ── PARTIAL mode preview ──────────────────────────────────────────────────
  // For partial customers, calculate the combined gross across all active tranches
  // using calcPartialBaseRent (same formula as financial.controller and generate).
  if (payMode === 'partial') {
    const parsed = parseFinancialPartials(cust.partial_payments);
    if (!parsed) {
      return {
        error: `Payment mode is 'partial' but no valid partial_payments data found for unit ${cust.unit_no}. Please update the financial record.`,
        code: 'MISSING_PARTIAL_DATA',
      };
    }
    ...
    if (parsed.type === 'financial') {
      ...
      if (!entries.length) {
        return {
          error: `No active partial tranches for unit ${cust.unit_no} in ${toMonthLabel(rentMonth)}.`,
          code: 'NO_ACTIVE_TRANCHES',
        };
      }
      ...
      if (!cdfg) {
        for (const e of allEntries) {
          const src = getEntryClosureDate(e); if (src) { cdfg = new Date(src); break; }
        }
      }
      const { escalationRate, yearsElapsed, monthlyRent } = computeGrossForCustomer(cust, cdfg, rentMonth);
      ...
      const entryData = entries.map((e) => {
        ...
        const amtRcvd  = bank + tdsRcvd;
        const closureS = getEntryClosureDate(e) ?? '';
        const baseRent   = calcPartialBaseRent(amtRcvd, closureS, totalSale, sqft, rentPerSft, rentMonth);
        const entryGross = round2(baseRent + (escalationRate > 0 ? round2(baseRent * escalationRate / 100) : 0));
        return { baseRent, entryGross, closureS };
      });

      const grossAmount = round2(entryData.reduce((s, d) => s + d.entryGross, 0));
      const tdsAmount   = tdsExempt ? 0 : calculateTDS(grossAmount);
      const netPayout   = round2(grossAmount - tdsAmount);
      const gd          = buildGstDetails(netPayout);
      ...
      return {
        data: {
          dbCustomerId: cust.id, customerId: cust.customer_id, customerName: cust.customer_name,
          customerUnitId: cust.customer_unit_id, unitNo: cust.unit_no, floorNo: cust.floor_no,
          agreementType: cust.agreement_type, tdsApplicable: cust.tds_applicable, tdsExempt,
          actualOccupancyDate: cust.actual_occupancy_date, paymentClosureDate: cust.payment_closure_date || null,
          paymentDate, rentMonth, paymentMonth: rentMonth,
          payoutSplits: splits, payoutBreakdown, paymentMode: 'partial',
          grossAmount, tdsAmount, tdsApplied: tdsAmount > 0, tdsThreshold: 50000,
          netPayout, baseRent: grossAmount, tdsRate: tdsAmount > 0 ? 10 : 0,
          escalationRate, yearsElapsed: round2(yearsElapsed),
          trancheCount: entries.length,
          rentCalculationDetails: {
            totalSaleConsideration: totalSale, totalAlreadyPaid, remainingBalance,
            sqft, rentalValuePerSft: rentPerSft, monthlyRent,
            activeEntries: entryData.length,
            note: `Partial payment: ${entries.length} active tranche(s) for ${toMonthLabel(rentMonth)}`,
          },
          ...gd, netBankTransfer: round2(netPayout + gd.totalGstAmount),
        },
      };
    }
  }
  ...
  return {
    data: {
      ...
      actualOccupancyDate: cust.actual_occupancy_date, paymentClosureDate: cust.payment_closure_date || null,
      paymentDate, rentMonth, paymentMonth: rentMonth,
      payoutSplits: splits, payoutBreakdown, paymentMode: 'full',
      grossAmount: round2(grossAmount), tdsAmount, tdsApplied: tdsAmount > 0, tdsThreshold: 50000,
      netPayout, baseRent: round2(grossAmount), tdsRate: tdsAmount > 0 ? 10 : 0,
      escalationRate, yearsElapsed: round2(yearsElapsed),
      rentCalculationDetails: {
        totalSaleConsideration: totalSale, totalAlreadyPaid, remainingBalance,
  ...
  try {
    ...
    await transaction(async (client) => {
      let cq = `
        ...
        FROM customers c
        JOIN customer_units cu
          ON cu.customer_id = c.id AND cu.deleted_at IS NULL AND cu.status = 'Active'
        ${frJoinForUnit}
        WHERE c.deleted_at IS NULL AND c.status = 'Active'
      `;
      const cp = [];
      ...
      for (const cust of customers) {
        ...
        const skip = (r) => skipped.push({
          ...
          unitId: cust.customer_unit_id, unitNo: cust.unit_no, reason: r,
        });
        ...
        // ── Duplicate guard (full mode only — partial uses per-row insertPayment guard) ──
        if (payMode !== 'partial') {
          const { rows: dup } = await client.query(
            `SELECT id FROM payments
             WHERE customer_unit_id = $1 AND payment_month = $2
               AND installment_no IS NULL AND status <> 'Cancelled' AND deleted_at IS NULL`,
            [cuId, rentMonth]
          );
          if (dup.length) {
            duplicates.push({ customerId: cust.id, customerName: cust.customer_name, unitId: cuId, unitNo: cust.unit_no, reason: `Payment already exists for ${toMonthLabel(rentMonth)}` });
            continue;
          }
        }

        // ── PARTIAL mode ─────────────────────────────────────────────────────
        // CRITICAL: if payMode === 'partial', we MUST use _processPartialCustomer.
        // We never fall through to full mode for a partial customer — doing so
        // would generate sqft × rentPerSFT (e.g. ₹36,000) instead of the
        // correct proportional tranche amount (e.g. ₹1,200).
        if (payMode === 'partial') {
          await _processPartialCustomer(client, {
            cust, rentMonth, scheduledDate: null, month, pmStr: null,
            tdsExemptC, splitsJson, cuId, period, userId, payments, skip,
          });
          continue;   // ← explicit continue: NEVER falls through to full mode
        }
    ...
    res.status(201).json({
      ...
      data: {
        initiationMonth: month, rentMonth,
        rentMonthDisplay: toMonthLabel(rentMonth), initiationMonthDisplay: toMonthLabel(month),
        paymentsGenerated: payments.length, skippedCount: skipped.length, duplicateCount: duplicates.length,
        payments, skipped, duplicates,
      },
    });
  } catch (err) {
    ...
    await transaction(async (client) => {
      for (const customerId of customerIds) {
        ...
        for (const cust of rows) {
          ...
          const skip = (r) => skipped.push({ customerId, customerName: cust.customer_name, unitId: cust.customer_unit_id, unitNo: cust.unit_no, reason: r });
          const tdsExemptC = isTdsExempt(cust);
          const splitsJson = cust.payout_splits
            ? (typeof cust.payout_splits === 'string' ? cust.payout_splits : JSON.stringify(cust.payout_splits))
            : null;
          ...
          const startMonthKey = toMonthKey(getEffectiveStartDate(cust));
          if (startMonthKey && rentMonth < startMonthKey) { skip(`Payment starts ${toMonthLabel(startMonthKey)}`); continue; }
          ...
          // ── PARTIAL mode — same safety as generateMonthlyPayments ──────────
          if (payMode === 'partial') {
            await _processPartialCustomer(client, {
              cust, rentMonth, scheduledDate, month: null, pmStr,
              tdsExemptC, splitsJson, cuId, period, userId, payments, skip,
            });
            continue;   // ← explicit continue: NEVER falls through to full mode
          }
  ...
  try {
    ...
    let sql = `
      SELECT
        ...
        COALESCE(cu_direct.agreement_type, c.agreement_type) AS agreement_type,
        ${UNIT_COLS},
        fr.rent AS financial_rent, fr.tds_applicable,
        fr.rental_value_per_sft, fr.total_sale_consideration,
        fr.payment_closure_date, fr.payment_mode
      FROM payments p
      JOIN customers c ON p.customer_id = c.id
      ${UNIT_JOIN}
      ${FR_JOIN_FOR_PAYMENT}
    ...
    if (monthNo)        { sql += ` AND SUBSTRING(p.payment_month, 6, 2) = $${pi}`;             params.push(String(monthNo).padStart(2, '0')); pi++; }
    if (status)         { sql += ` AND p.status = $${pi}`;                                      params.push(status);                           pi++; }
    if (agreementType)  { sql += ` AND COALESCE(cu_direct.agreement_type, c.agreement_type) = $${pi}`; params.push(agreementType);            pi++; }
  ...
  try {
    ...
    const { rows } = await query(
      `SELECT p.*, c.customer_name, c.customer_id AS customer_code,
         c.email, c.phone, c.pan_number,
         c.agreement_type AS customer_agreement_type, c.gst_no, c.cgst, c.sgst,
         COALESCE(cu_direct.agreement_type, c.agreement_type) AS agreement_type,
         ${UNIT_COLS},
         fr.rent AS financial_rent, fr.tds_applicable,
         fr.rental_value_per_sft, fr.total_sale_consideration,
...
const completePayment = async (req, res) => {
  try {
    ...
    if (!rows.length)
      return res.status(404).json({ success: false, error: 'Payment not found or not in Processing status' });
...
const initiatePaymentBatch = async (req, res) => {
  try {
    ...
    await transaction(async (client) => {
      const { rows: pmts } = await client.query(
        `SELECT * FROM payments WHERE id = ANY($1) AND status = 'Pending' AND deleted_at IS NULL`,
        [paymentIds]
      );
      if (!pmts.length) throw new Error('No valid pending payments found');
      ...
      for (let i = 0; i < pmts.length; i++) {
        await client.query(
          `INSERT INTO payment_batch_items (batch_id, payment_id, sequence_number) VALUES ($1,$2,$3)`,
          [batch.id, pmts[i].id, i + 1]
        );
        await client.query(
          `UPDATE payments SET status='Processing', processed_by=$1, processed_date=NOW() WHERE id=$2`,
          [userId, pmts[i].id]
        );
      }
  ...
  try {
    ...
    let queryText = `
      SELECT
        ...
        c.agreement_type                 AS customer_agreement_type,
        c.tds_applicable                 AS customer_tds_applicable,
        c.nri_status, c.gst_no, c.cgst, c.sgst,
        COALESCE(cu_direct.agreement_type, c.agreement_type) AS agreement_type,
        ${UNIT_COLS},
        fr.rent AS financial_rent, fr.tds_applicable AS fr_tds_applicable,
        fr.rental_value_per_sft, fr.total_sale_consideration,
        fr.payment_closure_date, fr.payment_mode
      FROM payments p
      JOIN customers c ON p.customer_id = c.id
    ...
    if (endDate)       { queryText += ` AND p.payment_date <= $${pi}`; queryParams.push(endDate);     pi++; }
    if (month)         { queryText += ` AND p.payment_month = $${pi}`; queryParams.push(month);       pi++; }
    if (agreementType) { queryText += ` AND COALESCE(cu_direct.agreement_type, c.agreement_type) = $${pi}`; queryParams.push(agreementType); pi++; }
    ...
    if (endDate)       { countQuery += ` AND p.payment_date<=$${cp}`; countParams.push(endDate);     cp++; }
    if (month)         { countQuery += ` AND p.payment_month=$${cp}`; countParams.push(month);       cp++; }
    if (agreementType) { countQuery += ` AND COALESCE(cu_direct.agreement_type, c.agreement_type)=$${cp}`; countParams.push(agreementType); cp++; }
...
const savePaymentWithAdjustment = async (req, res) => {
  try {
    ...
    const dupCheck = customerUnitId
      ...
      : await query(
          `SELECT id, status FROM payments
           WHERE customer_id = $1 AND customer_unit_id IS NULL AND payment_month = $2
             AND status <> 'Cancelled' AND deleted_at IS NULL`,
          [customerId, rentMonth]
        );
    ...
    if (dupCheck.rows.length)
      return res.status(409).json({
        success: false,
        error: `A payment for ${rentMonth} already exists with status "${dupCheck.rows[0].status}". Cancel it first.`,
        code: 'PAYMENT_ALREADY_EXISTS', existingId: dupCheck.rows[0].id,
      });
    ...
    const { rows: [payment] } = await query(
      `INSERT INTO payments (
       ...
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9,0,0,$10,'Pending',$11,$12,$13,$14,$15::jsonb)
       RETURNING *`,
      [customerId, customerUnitId || null, paymentDate, rentMonth, round2(grossAmount), round2(tdsAmount), adjNet, period, round2(grossAmount), paymentDate, userId, adj, adjNet, noteWithAdj, splitsJson]
    );

    await query(
      `INSERT INTO audit_logs (user_id, action, resource_type, resource_id, changes, ip_address, user_agent, status)
       VALUES ($1,'PAYMENT_SAVED_WITH_ADJUSTMENT','PAYMENT',$2,$3,$4,$5,'SUCCESS')`,
      [userId, payment.id, JSON.stringify({ customerId, customerUnitId, rentMonth, paymentDate, grossAmount: round2(grossAmount), tdsAmount: round2(tdsAmount), originalNet: origNet, adjustmentAmount: adj, adjustedNet: adjNet, adjustmentNote, hasSplits: !!payoutSplits }), req.ip || '0.0.0.0', req.headers['user-agent'] || 'system']
    );

    res.status(201).json({
      success: true,
      message: `Payment saved with adjustment of ${adj >= 0 ? '+' : ''}₹${adj}. Adjusted net = ₹${adjNet}.`,
      data: { ...payment, original_net_payout: origNet, adjustment_amount: adj, adjusted_net_payout: adjNet, payout_breakdown: payoutBreakdown },
    });
  } catch (error) {
    console.error('savePaymentWithAdjustment error:', error);
    res.status(500).json({ success: false, error: 'Failed to save payment with adjustment' });
...
const getPaymentByMonth = async (req, res) => {
  try {
    const { customerId, customerUnitId, rentMonth } = req.query;
    if ((!customerId && !customerUnitId) || !rentMonth)
      return res.status(400).json({ success: false, error: 'customerId (or customerUnitId) and rentMonth are required' });

    const { rows } = customerUnitId
      ? await query(
          `SELECT p.*, c.customer_name, c.agreement_type
           FROM payments p JOIN customers c ON c.id = p.customer_id
           WHERE p.customer_unit_id = $1 AND p.payment_month = $2 AND p.deleted_at IS NULL
           ORDER BY p.created_at DESC LIMIT 1`,
          [customerUnitId, rentMonth]
        )
      : await query(
          `SELECT p.*, c.customer_name, c.agreement_type
           FROM payments p JOIN customers c ON c.id = p.customer_id
           WHERE p.customer_id = $1 AND p.payment_month = $2 AND p.deleted_at IS NULL
           ORDER BY p.created_at DESC LIMIT 1`,
          [customerId, rentMonth]
        );
...
const getSavedAdjustments = async (req, res) => {
  try {
    ...
    values.push(Math.min(200, parseInt(limit) || 50), Math.max(0, parseInt(offset) || 0));
    const { rows } = await query(
      `SELECT p.id, p.customer_id, p.customer_unit_id, p.payment_month, p.payment_date,
              p.gross_amount, p.tds_amount, p.net_payout, p.adjustment_amount,
              p.adjusted_net_payout, p.adjustment_note, p.status, p.created_at,
              c.customer_name
       FROM payments p JOIN customers c ON c.id = p.customer_id
