...
const validateUpsertBody = (body) => {
  const {
    customerUnitId, totalSaleConsideration, rentalValuePerSFT,
    paymentMode, bankCollection, dateOfPayment, partialPayments,
  } = body;

  if (!customerUnitId)
    return 'Customer unit ID is required';
  if (!totalSaleConsideration || toFloat(totalSaleConsideration) <= 0)
    return 'Total sale consideration must be a positive number';
  if (!rentalValuePerSFT || toFloat(rentalValuePerSFT) <= 0)
    return 'Rental value per SFT must be a positive number';
...
/* ─── UPSERT (one financial record per unit) ──────────────────────────────── */
const upsertFinancialRecord = async (req, res) => {
  try {
    const {
      customerUnitId,
      totalSaleConsideration, rentalValuePerSFT,
      paymentMode, paymentClosureDate,
      bankCollection, tdsCollection, dateOfPayment,
      tdsApplicableOverride, partialPayments,
    } = req.body;
    ...
    // Verify unit exists and pull sqft
    const unitCheck = await query(
      `SELECT cu.id, cu.sqft, c.customer_name, c.id AS customer_id_fk
    ...
    const unitSqft      = toFloat(unitCheck.rows[0].sqft);
    const customerIdFk  = unitCheck.rows[0].customer_id_fk;
    const totalSaleNum  = toFloat(totalSaleConsideration);

    let computedBankCollection, computedTdsCollection, computedTotalReceived,
        computedReceivedPct, computedRent, computedTdsApplicable,
        computedDateOfPayment, computedPartialPayments = null;
    ...
    if (paymentMode === 'full') {
      ...
      computedTdsCollection  = round2(tds);
      computedTotalReceived  = round2(bank + tds);
      computedReceivedPct    = totalSaleNum > 0 ? round2((computedTotalReceived / totalSaleNum) * 100) : 0;
      computedRent           = calcFullRent(unitSqft, rentalValuePerSFT);
      computedTdsApplicable  = resolveTdsApplicable(tdsApplicableOverride, computedRent);
      computedDateOfPayment  = dateOfPayment || null;
    } else {
      const enriched = partialPayments.map((p, idx) => {
        ...
        const amtReceived = round2(bank + tds);
        const closureDate = p.paymentClosureDate || null;
        const rent        = calcPartialRent(amtReceived, closureDate, totalSaleNum, unitSqft, rentalValuePerSFT);
        const rentTds     = calcTdsOnRent(rent);
        return {
          id:                 p.id ?? idx + 1,
          installment_no:     idx + 1,
          ...
          rent,
          rentTds,
          netRent: round2(rent - rentTds),
        };
      });

      computedBankCollection  = round2(enriched.reduce((s, p) => s + p.bankAmount,     0));
      computedTdsCollection   = round2(enriched.reduce((s, p) => s + p.tdsAmount,      0));
      computedTotalReceived   = round2(enriched.reduce((s, p) => s + p.amountReceived, 0));
      computedReceivedPct     = totalSaleNum > 0 ? round2((computedTotalReceived / totalSaleNum) * 100) : 0;
      computedRent            = round2(enriched.reduce((s, p) => s + p.rent, 0));
      computedTdsApplicable   = resolveTdsApplicable(tdsApplicableOverride, computedRent);
      computedDateOfPayment   = enriched.length > 0 ? (enriched[enriched.length - 1].date || null) : null;
      computedPartialPayments = JSON.stringify(enriched);
    }

    const params = [
      round2(totalSaleNum),
      round2(toFloat(rentalValuePerSFT)),
      paymentMode,
      computedBankCollection,
      computedTdsCollection,
      computedDateOfPayment,
      computedTotalReceived,
      computedReceivedPct,
      computedRent,
      computedTdsApplicable,
      paymentClosureDate || null,
      computedPartialPayments,
    ];

    // Check if a record already exists for this unit
    const existing = await query(
      `SELECT id FROM financial_records
       WHERE customer_unit_id = $1 AND deleted_at IS NULL`,
      [customerUnitId]
    );
    ...
    if (existing.rows.length > 0) {
      result = await query(
        `UPDATE financial_records SET
           ...
           updated_by               = $13,
           updated_at               = NOW()
         WHERE customer_unit_id = $14 AND deleted_at IS NULL
         RETURNING *`,
        [...params, userId, customerUnitId]
      );
    } else {
      // Insert — include both customer_unit_id AND customer_id (original column kept)
      result = await query(
        `INSERT INTO financial_records (
           total_sale_consideration, rental_value_per_sft, payment_mode,
           bank_collection, tds_collection, date_of_payment,
         ...
         RETURNING *`,
        [...params, customerUnitId, customerIdFk, userId]
      );
    }
    ...
    console.error('[upsertFinancialRecord]', err);
    return res.status(500).json({ success: false, error: 'Failed to save financial record' });
  }
};
  ...
  try {
    ...
    if (search) {
      where += ` AND (
        c.customer_name  ILIKE $${pi} OR
        c.customer_id    ILIKE $${pi} OR
        cu.property_name ILIKE $${pi} OR
        cu.unit_ref      ILIKE $${pi}
      )`;
      params.push(`%${search}%`); pi++;
    }
    if (paymentMode && ['full', 'partial'].includes(paymentMode)) {
      where += ` AND fr.payment_mode = $${pi}`;
      params.push(paymentMode); pi++;
    }

    const [rows, countRes] = await Promise.all([
      query(
        `SELECT fr.*,
                cu.unit_ref, cu.property_name, cu.floor_no, cu.unit_no, cu.sqft,
                c.customer_id AS customer_ref, c.customer_name
         FROM financial_records fr
         JOIN customer_units cu ON fr.customer_unit_id = cu.id
         JOIN customers c       ON cu.customer_id = c.id
         ${where}
         ORDER BY fr.created_at DESC
         LIMIT $${pi} OFFSET $${pi + 1}`,
    ...
    return res.json({
      success: true,
      data: {
        records: rows.rows.map((r) => ({ ...r, partial_payments: parseJsonb(r.partial_payments) })),
        pagination: { page: pageNum, limit: limitNum, total, totalPages: Math.ceil(total / limitNum) },
      },
    });
  } catch (err) {
    console.error('[getAllFinancialRecords]', err);
    return res.status(500).json({ success: false, error: 'Failed to fetch financial records' });
  }
};
...
const getFinancialRecordByCustomer = async (req, res) => {
  try {
    // :customerId param now receives a customer_units.id (unit ID)
    const customerUnitId = req.params.customerId || req.params.customerUnitId;
    ...
    const result = await query(
      `SELECT fr.*,
       ...
       JOIN customer_units cu ON fr.customer_unit_id = cu.id
       JOIN customers c       ON cu.customer_id = c.id
       WHERE fr.customer_unit_id = $1 AND fr.deleted_at IS NULL
       LIMIT 1`,
      [customerUnitId]
    );
    ...
    console.error('[getFinancialRecordByCustomer]', err);
    return res.status(500).json({ success: false, error: 'Failed to fetch financial record' });
  }
};
...
const getFinancialRecordById = async (req, res) => {
  try {
    ...
    const result = await query(
      `SELECT fr.*,
              cu.unit_ref, cu.property_name, cu.sqft,
              c.customer_id AS customer_ref, c.customer_name
       FROM financial_records fr
       JOIN customer_units cu ON fr.customer_unit_id = cu.id
       JOIN customers c       ON cu.customer_id = c.id
    ...
    console.error('[getFinancialRecordById]', err);
    return res.status(500).json({ success: false, error: 'Failed to fetch financial record' });
  }
};
...
const deleteFinancialRecord = async (req, res) => {
  try {
    ...
    const userId = req.user?.id;
    const result = await query(
      `UPDATE financial_records SET deleted_at=NOW(), updated_by=$1
    ...
    console.error('[deleteFinancialRecord]', err);
    return res.status(500).json({ success: false, error: 'Failed to delete financial record' });
  }
};
...
const getFinancialSummary = async (_req, res) => {
  try {
    const [overall, tdsBreakdown] = await Promise.all([
      query(`
        SELECT
          ...
          COALESCE(ROUND(SUM(rent)::NUMERIC,2),0)                               AS total_rent,
          COUNT(*) FILTER (WHERE tds_applicable='Y')                            AS tds_applicable_count,
          COALESCE(ROUND(SUM(total_sale_consideration - total_received)
            FILTER (WHERE total_sale_consideration > total_received)::NUMERIC,2),0) AS total_outstanding
    ...
    return res.json({
      success: true,
      data: { summary: overall.rows[0], tdsBreakdown: tdsBreakdown.rows },
    });
  } catch (err) {
