...
const ForgotPassword = ({ onSuccess, onBack }) => {
  ...
  useEffect(() => {
    if (step === 1) setTimeout(() => emailRef.current?.focus(), 50);
  ...
  // ── Step 1: Send OTP ────────────────────────────────────────────────────────
  // ROOT CAUSE OF "Failed to send OTP" toast:
  // Previously the catch block fired on ANY error including network issues,
  // AND the success branch was not clearly separated.
  // Fix: explicit try/catch with proper error message extraction.
  const handleSendOTP = async (e) => {
    e.preventDefault();
    ...
    try {
      ...
      // SUCCESS path — only runs if no error thrown
      toast.success(res?.data?.message || 'OTP sent! Check your inbox.');
      setStep(2);
      startCountdown();
    } catch (err) {
      // FAILURE path — extract the most specific message available
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      const serverMsg = err?.response?.data?.error || err?.response?.data?.message;
      toast.error(serverMsg || 'Failed to resend OTP. Please try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleOtpKeyDown = (idx, e) => {
    ...
    if (e.key === 'ArrowLeft'  && idx > 0)              otpRefs.current[idx - 1]?.focus();
    if (e.key === 'ArrowRight' && idx < OTP_LENGTH - 1) otpRefs.current[idx + 1]?.focus();
    // Allow Enter to submit when all digits are filled
  ...
  const handleVerifyOTP = async (e) => {
    ...
    try {
      ...
      const serverMsg = err?.response?.data?.error || err?.response?.data?.message;
      toast.error(serverMsg || 'Invalid or expired OTP. Please try again.');
      setOtp(Array(OTP_LENGTH).fill(''));
      setTimeout(() => otpRefs.current[0]?.focus(), 50);
    } finally {
      setLoading(false);
  ...
  const handleResetPassword = async (e) => {
    ...
    try {
      ...
      const serverMsg = err?.response?.data?.error || err?.response?.data?.message;
      toast.error(serverMsg || 'Password reset failed. Please try again.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <>
      <style>{`
        ...
        .fp-wrap {
          width: 100%;
          max-width: 440px;
          animation: fpIn 0.3s ease forwards;
        }
        @keyframes fpIn {
        ...
        .fp-step-dot.active {
          ...
          border-color: #0f2d78;
          color: #fff;
          box-shadow: 0 0 0 4px rgba(15,45,120,0.12);
        }
        .fp-step-dot.done {
          background: #10b981;
          border-color: #10b981;
        ...
        .fp-step-dot.done i { font-size: 12px; }
        .fp-step-name {
          font-size: 0.62rem;
          font-weight: 600;
          color: #d1d5db;
        ...
        .fp-step-name.active { color: #0f2d78; }
        .fp-step-name.done   { color: #10b981; }
        .fp-step-line-wrap {
          flex: 1;
          padding-top: 15px;
        }
        ...
        .fp-input:focus {
          border-color: #0f2d78;
          box-shadow: 0 0 0 3px rgba(15,45,120,0.10);
        }
        .fp-input::placeholder { color: #c4c9d4; }
        .fp-toggle-pw {
          position: absolute;
        ...
        .fp-otp-cell::-webkit-outer-spin-button,
        .fp-otp-cell::-webkit-inner-spin-button { -webkit-appearance: none; }
        .fp-otp-cell:focus {
          border-color: #0f2d78;
          box-shadow: 0 0 0 3px rgba(15,45,120,0.12);
          transform: translateY(-2px);
        }
        .fp-otp-cell.fp-filled {
          border-color: #0f2d78;
          background: #f0f4ff;
          color: #0f2d78;
        }
        ...
        .fp-timer-val.expiring { color: #ef4444; }
        .fp-resend {
          background: none;
          border: none;
          cursor: pointer;
        ...
        .fp-resend:hover:not(:disabled) { color: #d4a017; }
        .fp-resend:disabled { color: #d1d5db; cursor: not-allowed; text-decoration: none; }
        ...
        .fp-email-chip i { font-size: 12px; flex-shrink: 0; }
        ...
        .fp-btn-primary {
          background: linear-gradient(135deg, #0f2d78 0%, #1a3fa0 100%);
          color: #fff;
          box-shadow: 0 4px 18px rgba(15,45,120,0.30);
        }
        .fp-btn-primary:hover:not(:disabled) {
          transform: translateY(-1px);
          box-shadow: 0 7px 22px rgba(15,45,120,0.38);
        }
        .fp-btn-primary:active:not(:disabled) { transform: translateY(0); }
        ...
        .fp-btn-success:hover:not(:disabled) {
          transform: translateY(-1px);
          box-shadow: 0 7px 22px rgba(16,185,129,0.38);
        }
        .fp-btn-success:active:not(:disabled) { transform: translateY(0); }
        .fp-btn-success:disabled { opacity: 0.58; cursor: not-allowed; }
        ...
        /* ── Step transition ── */
        .fp-step-body {
          animation: fpStepIn 0.25s ease forwards;
        }
        @keyframes fpStepIn {
      ...
      <div className="fp-wrap">
        ...
        <div className="fp-progress">
          {stepLabels.map((label, i) => {
            ...
            const isAct  = step === num;
            return (
              <React.Fragment key={num}>
                <div className="fp-step-col">
                  <div className={`fp-step-dot ${isDone ? 'done' : isAct ? 'active' : ''}`}>
                    {isDone ? <i className="bi bi-check-lg" /> : num}
                  </div>
                  <div className={`fp-step-name ${isDone ? 'done' : isAct ? 'active' : ''}`}>
                    {label}
                  </div>
                </div>
                {i < stepLabels.length - 1 && (
                  <div className="fp-step-line-wrap">
                    <div className={`fp-step-line ${isDone ? 'done' : ''}`} />
                  </div>
                )}
              </React.Fragment>
            );
        ...
        {/* ══ STEP 1: Email ══════════════════════════════════════════════════ */}
        {step === 1 && (
          <div key="s1" className="fp-step-body">
            <div className="fp-card-head">
              <div className="fp-eyebrow">Password Recovery</div>
              <h2 className="fp-title">Forgot your<br/>password?</h2>
              <p className="fp-subtitle">
                Enter your registered email and we'll send a 6-digit OTP to your inbox.
              </p>
            </div>
        ...
        {/* ══ STEP 2: OTP ════════════════════════════════════════════════════ */}
        {step === 2 && (
          <div key="s2" className="fp-step-body">
            <div className="fp-card-head">
              <div className="fp-eyebrow">Verification</div>
              <h2 className="fp-title">Enter the<br/>6-digit code</h2>
              <p className="fp-subtitle">We sent a one-time code to</p>
            </div>
              ...
              <div className="fp-otp-grid" onPaste={handleOtpPaste}>
                {otp.map((digit, idx) => (
                  <input
                    ...
                    inputMode="numeric"
                    maxLength={1}
                    className={`fp-otp-cell ${digit ? 'fp-filled' : ''}`}
                    value={digit}
                    onChange={e => handleOtpChange(idx, e.target.value)}
                    onKeyDown={e => handleOtpKeyDown(idx, e)}
                    aria-label={`OTP digit ${idx + 1}`}
              ...
              <div className="fp-timer-row">
                {countdown > 0 ? (
                  <>
                    ...
                    Code expires in&nbsp;
                    <span className={`fp-timer-val ${countdown <= 60 ? 'expiring' : ''}`}>
                  ...
                  <>
                    <i className="bi bi-exclamation-circle" style={{ fontSize: 13, color: '#ef4444' }} />
        ...
        {/* ══ STEP 3: New Password ════════════════════════════════════════════ */}
        {step === 3 && (
          <div key="s3" className="fp-step-body">
            <div className="fp-card-head">
              <div className="fp-eyebrow">Almost Done</div>
              <h2 className="fp-title">Set a new<br/>password</h2>
              <p className="fp-subtitle">
                Choose a strong, unique password for your account.
              </p>
            </div>
                  ...
                  <>
                    ...
                    <div className="fp-strength-rules">
                      {[
                        ...
                        <div key={rule} className={`fp-rule ${met ? 'met' : ''}`}>
                          <i className={`bi bi-${met ? 'check-circle-fill' : 'circle'}`} />
                          {rule}
                        </div>
                      ))}
                    </div>
