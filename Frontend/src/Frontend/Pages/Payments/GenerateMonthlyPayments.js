...
// Next month label (used in info banner)
...
const GenerateMonthlyPayments = () => {
  ...
  // ── Totals computed from result.payments ───────────────────────────────────
  // Backend enrichPayment() spreads computeGstForPayment() which returns:
  //   has_gst, gst_no, cgst_rate, sgst_rate,
  //   cgst_amount, sgst_amount, total_gst_amount, net_transfer
  //
  // net_transfer = net_payout + total_gst_amount  (the ONE final amount to disburse)
  // For non-GST customers net_transfer === net_payout (set explicitly in backend).
  const payments = result?.payments || [];

  const totalGross    = round2(payments.reduce((s, p) => s + parseFloat(p.gross_amount     || 0), 0));
  const totalTds      = round2(payments.reduce((s, p) => s + parseFloat(p.tds_amount       || 0), 0));
  const totalNet      = round2(payments.reduce((s, p) => s + parseFloat(p.net_payout       || 0), 0));
  const totalCgst     = round2(payments.reduce((s, p) => s + parseFloat(p.cgst_amount      || 0), 0));
  const totalSgst     = round2(payments.reduce((s, p) => s + parseFloat(p.sgst_amount      || 0), 0));
  // net_transfer is the authoritative final figure (includes GST when applicable).
  // Fall back to net_payout for rows where net_transfer may be absent (legacy rows).
  const totalTransfer = round2(payments.reduce((s, p) => s + parseFloat(p.net_transfer ?? p.net_payout ?? 0), 0));

  // Whether any payment in the result set has GST
  const hasAnyGst = payments.some((p) => p.has_gst);

  // Column counts for tfoot colSpan calculation
  // Fixed leading cols: #, Customer, Unit No, Floor, Period, Base Rent, Esc. = 7
  // Then: Gross, TDS, Net Rent [, CGST, SGST] [, Net Transfer/Payout]
  // The "Totals" label spans the first 7 fixed cols.
  const FIXED_COLS = 7;
  ...
  return (
    <div className="content-area generate-payments-container">
      <div className="generate-payments-header">
        <h4>
          <i className="bi bi-calendar-plus generate-icon me-2" />
          Generate Monthly Payments
        </h4>
        <p className="generate-subtitle">Create rent payments for all active customers</p>
      </div>
      ...
      <div className="generate-card">
        <div className="generate-card-body">
          <div className="row g-4">
            <div className="col-md-4">
              <label className="form-label fw-semibold">
                <i className="bi bi-calendar3 me-2" />Initiation Month
              </label>
              <input
                type="month"
                className="form-control form-control-lg"
            ...
            <div className="col-md-4">
              <label className="form-label fw-semibold">
                <i className="bi bi-file-earmark-text me-2" />Agreement Type
              </label>
              <select
                className="form-select form-select-lg"
                name="agreementType"
          ...
          {/* Month mapping info banner */}
          {formData.month && (
            <div className="alert alert-info mt-3 mb-0 d-flex align-items-start gap-2">
              <i className="bi bi-info-circle-fill fs-5 mt-1" />
              <div>
                <strong>How month mapping works:</strong><br />
                Selecting <strong>{fmtMonth(formData.month)}</strong> as the initiation month will generate
                rent payments for <strong className="text-primary">{fmtMonth(rentMonthPreview)}</strong>.
                Payments are always generated for the <em>previous calendar month</em>.
                Customers whose payment started in <strong>{fmtMonth(formData.month)}</strong> will be
                included when you select <strong>{fmtMonth(getNextMonth(formData.month))}</strong> as
        ...
        <div className="generate-results mt-4">
          ...
          <div className="row g-3 mb-4">
            {[
              ...
              <div className="col-md-3" key={label}>
                <div className={`summary-card ${cls}`}>
                  <div className="summary-icon"><i className={`bi bi-${icon}`} /></div>
                  <div className="summary-content">
                    <div className="summary-value">{value}</div>
                    <div className="summary-label">{label}</div>
                  </div>
                  ...
                  <table className="table table-hover mb-0 align-middle">
                    <thead className="table-light">
                      <tr>
                        ...
                        {/* Single final disbursement column */}
                        <th className="table-primary fw-bold">
                          {hasAnyGst ? 'Net Transfer' : 'Net Payout'}
                        </th>
                      </tr>
                    ...
                    <tbody>
                      {payments.map((payment, idx) => (
                        <tr key={payment.id || idx}>
                          ...
                          <td>{formatCurrency(payment.base_rent)}</td>
                          <td>
                            {parseFloat(payment.escalation_rate) > 0
                              ? `${payment.escalation_rate}%`
                              : <span className="text-muted">—</span>}
                          </td>
                          <td>{formatCurrency(payment.gross_amount)}</td>
                          ...
                          {hasAnyGst && (
                            <td className="text-info">
                              {payment.has_gst
                                ? formatCurrency(payment.cgst_amount)
                                : <span className="text-muted">—</span>}
                            </td>
                          )}
                          {hasAnyGst && (
                            <td className="text-info">
                              {payment.has_gst
                                ? formatCurrency(payment.sgst_amount)
                                : <span className="text-muted">—</span>}
                            </td>
                          )}
                          {/* net_transfer is the final disbursement amount (net_payout + GST).
                              Fall back to net_payout when net_transfer is absent (non-GST rows). */}
                          <td className="fw-bold text-success table-primary">
                            {formatCurrency(payment.net_transfer ?? payment.net_payout)}
                          </td>
                        </tr>
                    ...
                    <tfoot className="table-secondary fw-bold">
                      <tr>
                        {/* FIXED_COLS (7) leading columns → "Totals" label */}
                        <td colSpan={FIXED_COLS} className="text-end">Totals</td>
                        <td>{formatCurrency(totalGross)}</td>
                        <td className="text-warning">{formatCurrency(totalTds)}</td>
                        <td>{formatCurrency(totalNet)}</td>
                        {hasAnyGst && <td className="text-info">{formatCurrency(totalCgst)}</td>}
                        {hasAnyGst && <td className="text-info">{formatCurrency(totalSgst)}</td>}
                        {/* The one authoritative transfer total */}
                        <td className="text-success table-primary fs-6">
          ...
          {result.skipped?.length > 0 && (
            <div className="card mb-3">
              ...
              <div className="card-body p-0">
                <table className="table table-sm mb-0">
                  <thead className="table-light">
                    <tr><th>Customer</th><th>Unit</th><th>Reason</th></tr>
                  </thead>
                  <tbody>
                    {result.skipped.map((item, i) => (
                      <tr key={i}>
          ...
          <div className="d-flex justify-content-end gap-2">
            <button className="btn btn-outline-secondary" onClick={() => setResult(null)}>
              <i className="bi bi-arrow-clockwise me-2" />Generate Again
            </button>
            <button
              className="btn btn-primary"
              onClick={() => navigate(`/payments/schedule?month=${formData.month}`)}
            >
              <i className="bi bi-calendar-check me-2" />View Payment Schedule
            </button>
          </div>
        </div>
      )}
      ...
      <div className="card mt-4">
        <div className="card-header bg-light">
          <h6 className="mb-0"><i className="bi bi-info-circle me-2" />How It Works</h6>
        </div>
        <div className="card-body small">
          <ul className="mb-0">
            <li className="mb-1">
              <strong>Month mapping:</strong> Selecting <em>February</em> generates rent payments
              for <em>January</em>. Always the previous month.
            </li>
            <li className="mb-1">
              <strong>New customers:</strong> If a customer's payment started in February, they
              appear when you select <em>March</em> as the initiation month.
            </li>
            <li className="mb-1">
              <strong>Partial payments:</strong> Each active tranche is computed separately;
              TDS applies on the combined total.
            </li>
            <li className="mb-1">
              <strong>9-Year escalation:</strong> 0% (0–3 yrs), 15% (3–6 yrs), 32.25% (6+ yrs)
              — floor 7 only.
            </li>
            <li className="mb-1">
              <strong>TDS:</strong> 10% auto-deducted on amounts ≥ ₹50,000 unless marked exempt.
            </li>
            <li>
              <strong>GST:</strong> CGST + SGST (18% total by default) applied on Net Rent for
              customers with a GST number. <strong>Net Transfer = Net Rent + GST</strong>.
            </li>
          </ul>
        </div>
      </div>
